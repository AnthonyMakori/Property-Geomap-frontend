...
function VacantHouses() {
  const { data: session, status } = useSession();
    const [searchTerm, setSearchTerm] = useState('');
    const [debouncedSearchTerm, setDebouncedSearchTerm] = useState('');
    const [pdfLoading, setPdfLoading] = useState({});
  
    const vacantUnitsStatus = useSelector((state) => state.reports.getVacantUnitsStatus);
    const vacantUnits = useSelector((state) => state.reports.getVacantUnits);
  
    const isLoading = vacantUnitsStatus === 'loading';
            ...
            <AppLayout>
                <Container fluid>
                    ...
                    <Flex
                            ...
                            >
                                <Stack>
                                    <Title order={3}>Vacant Units Report</Title>
                                </Stack>
                            </Flex>
                        <Paper p="md" shadow='md' radius="md">
                            <Group position="apart" mb="md">
                                <Text fz="lg" fw={600}>Total Units: {vacantUnits?.total ?? 0}</Text>
