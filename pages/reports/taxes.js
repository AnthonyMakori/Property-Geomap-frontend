...
import { getTaxes } from "@/store/reports/reports-slice";
...
function TaxReport() {
  ...
  const [isLoadingExcel, setIsLoadingExcel] = useState(false);
    ...
    const taxesStatus = useSelector((state) => state.reports.getTaxesStatus);
    const taxes = useSelector((state) => state.reports.getTaxes);
  
    const isLoading = taxesStatus === 'loading';
                ...
                <Container fluid>
                    <Stack spacing="lg">
                        ...
                        <Group position="apart" mb="md">
                                <Text fz="lg" fw={600}>Tax Report</Text>
                            ...
                            <tbody>
                            {taxes?.items?.data?.map((item) => (
                            <tr key={item?.id} >
                            <td>#{ item?.invoice?.code ?? '-' }</td>
                            <td>{item?.sale?.contact?.name ?? item?.invoice?.tenant?.name ?? "-"}</td>
                            {/* <td>{item?.land?.land_id ?? "-"}</td>
                            ...
                            <td>{new Date(item?.created_at).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric', hour: '2-digit', minute: '2-digit' })}</td>
                             ...
                             <tr className="text-lg">
                              ...
                              <td className="text-dark tracking-wider text-xxl font-bold">
                                Ksh. {formatNumber(taxes?.total) ?? 0}
                              </td>
                            </tr>
                            </tbody>                           
                            </Table>
